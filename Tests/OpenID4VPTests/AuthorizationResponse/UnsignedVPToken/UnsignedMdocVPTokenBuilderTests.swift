...
final class UnsignedMdocVPTokenBuilderTests: XCTestCase {
    ...
    func testCreationOfUnsignedMdocVPTokenForDraft23() async throws {
        // draft23: handover = [sha256(clientId+nonce), sha256(responseUri+nonce), nonce]
    ...
    func testCreationOfUnsignedMdocVPToken() async throws {
        // spec v1: handover = ["OpenID4VPHandover", sha256(clientId+nonce+jwkThumbprint+responseUri)]
        let builder = UnsignedMdocVPTokenBuilder(
    ...
    func testDeviceAuthenticationBytesAreDifferentBetweenSpecVersions() async throws {
        // The handover construction differs per spec version, so the resulting bytes must differ
        var mappingsDraft23 = [
            CredentialInputDescriptorMapping(format: .mso_mdoc, credential: AnyCodable(sampleMdoc), inputDescriptorId: "id-1")
        ]
        var mappingsV1 = [
        ...
        let (_, draft23Token) = try await UnsignedMdocVPTokenBuilder(
            authorizationRequest: getMockAuthorizationRequest(specVersion: .draft23),
            specVersion: .draft23,
            responseUri: "response-uri",
            mdocGeneratedNonce: "mock-nonce"
        ...
        let (_, v1Token) = try await UnsignedMdocVPTokenBuilder(
            authorizationRequest: getMockAuthorizationRequest(specVersion: .v1),
            specVersion: .v1,
            responseUri: "response-uri",
            mdocGeneratedNonce: "mock-nonce"
        ...
        let draft23Bytes = (draft23Token as! UnsignedMdocVPToken).docTypeToDeviceAuthenticationBytes["org.iso.18013.5.1.mDL"]
        let v1Bytes = (v1Token as! UnsignedMdocVPToken).docTypeToDeviceAuthenticationBytes["org.iso.18013.5.1.mDL"]
