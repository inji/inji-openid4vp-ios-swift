...
final class MdocVPTokenBuilderTests: XCTestCase {
    ...
    func testBuildsVPTokenSuccessfullyWithValidInput() {
        ...
        let builder = MdocVPTokenBuilder(mdocVPTokenSigningResult: metadata, unsignedMdocVPToken: unsignedToken, credentials: credentials)
    ...
    func testThrowsErrorWhenDeviceAuthenticationBytesAreMissing() {
        ...
        XCTAssertThrowsError(try builder.build()) { error in
            XCTAssertEqual(error.localizedDescription, "Missing Input: mdocVPTokenSigningResult->deviceAuthenticationBytesSigned->DeviceAuthentication param is required")
        }
    }
