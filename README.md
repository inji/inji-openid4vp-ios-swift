...
inji-openid4vp-ios-swift is an implementation of OpenID for Verifiable Presentations written in 

## Key Concepts

- **Verifier**: An entity that requests a Verifiable Presentation from the Holder (Wallet) and verifies it. In the context of this library, the Verifier is represented as an object containing its client ID, response URIs, JWKS URI, and whether it allows unsigned requests.
- **Wallet**: An entity that holds Verifiable Credentials and can create Verifiable Presentations to share with Verifiers. The Wallet interacts with the OpenID4VP library to process authorization requests and generate responses.

- **Authorization Request*: A request sent by the Verifier to the Wallet, asking for a Verifiable Presentation. It contains information about the requested credentials, response type, client ID, and other parameters.
  
- **Presentation**: A structured format that contains the Verifiable Credentials selected by the user in response to the Authorization Request. It is generated by the Wallet and sent back to the Verifier.
- **Holder Binding**: A mechanism to bind the Verifiable Presentation to the holder (the entity that owns the credentials) to prevent misuse. 
...
## Callout
1. Removal of support for OpenID for Verifiable Presentations - draft 21 support
...
| Client id prefix                                           | `pre-registered`, `redirect_uri`, `decentralized_identifier` (or `did`)                                                                                                                                                                                                                                                                           |
