...
public class OpenID4VP {
    ...
    public func shareVerifiablePresentation(vpResponseMetadata: VPResponseMetadata) async throws -> String? {
        ...
        do {
            return try await AuthorizationResponse.shareVp(vpResponseMetadata: vpResponseMetadata,authorizationRequest: authorizationRequest!, networkManager: networkManager)
        } catch(let exception) {
            await sendErrorToVerifier(error: exception)
            throw exception
        }
